...
class HomeController {
    ...
    orderProduct = async (req: Request, res: Response) => {
        if (req.session.User) {
            ...
            let cart = await userService.orderProduct(req.body.quantity, req.params.id, req.session.User);
            res.redirect(301, '/home-customer');
        }
        else {
            res.redirect(301, '/users/login');
    ...
    showFormCart = async (req: Request, res: Response) => {
        let user = await this.userService.findById(req.session.User);
        console.log(user);
        let cart = await userService.findCartByUser(req.session.User, req.params.);
        console.log(cart);
