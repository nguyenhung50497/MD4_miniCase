...
class HomeController {
    ...
    orderProduct = async (req: Request, res: Response) => {
        if (req.session.User) {
            ...
            let cart = await userService.orderProduct(product, user);
            res.redirect(301, '/home-customer');
        }
        else {
            res.redirect(301, '/users/login');
    ...
    searchBlog = async (req: { query: { keyword: any; }; },res: { status: (arg0: number) => { (): any; new(): any; json: { (arg0: Omit<Document<unknown, any, IProduct> & IProduct & { _id: Types.ObjectId; }, never>[]): void; new(): any; }; }; }) => {
        let blog = await productService.search(req.query.keyword);
