    ...
    priceRange1 = async (value) => {
        ...
        switch (value) {
            case 100:
                ...
                return products;
            case 500:
                products = await Product.find({ $and: [{price: {$gte: 101}}, {price: {$lte: value}}]}).populate('category');
                if (!products) {
                    return null;
                }
                return products;
            case 1000:
                products = await Product.find({ $and: [{price: {$gte: 501}}, {price: {$lte: value}}]}).populate('category');
                if (!products) {
                    return null;
                }
                return products;
